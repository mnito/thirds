import '../lib/ed448.dart';

List<int> _toBE(BigInt n, int numBytes) {
...
test() {
  ...
  for (final v in testVectors) {
    var secret = parseKeyString(v[0]);

    assert(toHexString(secret) == v[0], "Bad secret parsing");

    final publicBytes = secretToPublic(secret);
  ...
  assert(SIGNATURE_LENGTH == 114, "Bad signature length constant");
